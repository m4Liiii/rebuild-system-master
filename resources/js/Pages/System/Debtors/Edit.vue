      ...
      <div class="p-4 mx-4 rounded-lg shadow-md bg-zinc-700">
        <div class="flex flex-wrap justify-center flex-initial m-2">
          ...
          <div class="px-2 my-3 lg:w-1/2">
            <InputLabel for="name_debtor4" value="ناوی قەرزارە" class="relative" />
            <TextInput
              id="name_debtor4"
              type="text"
              maxLength="35"
              ...
              class="w-full mt-1"
              placeholder="ناوی قەرزارە"

            />
            <InputError class="mt-2"  :message="form.errors.name_debtor ? 'هەڵە هەیە لە ناوی قەرزارە' : ''"/>
          </div>
...
const form = useForm({
    name_debtor: props.debtor.name_debtor,
    address: props.debtor.address,
    phone_number: props.debtor.phone_number,
    social_number: props.debtor.social_number,
    notice: props.debtor.notice,
});

const submit = () => {
  form.put(route("debtors.update" , props.debtor.id));
};
