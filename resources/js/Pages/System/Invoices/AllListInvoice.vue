    ...
    <div class="p-6">
      <div class="flex justify-center">
        ...
        <div class="w-full p-2 px-10 mx-auto ">
            <InputLabel for="NameCust" value="گەڕان بۆ کڕیاری کۆ" />
            <searchable-select
            :items="customers"
            :label="'گەڕان بۆ کڕیاری کۆ'"
            v-model="searchCustId"
            :searchFields="['name_customer','phone_number','address']"
            class="w-full"
            :deselectable="true"
           ...
           >
            <template #option-content="{ option }">
                {{ option.name_customer }} -  {{ option.phone_number }} - {{ option.address }}
            </template>
           </searchable-select>
        </div>

        <div class="w-full p-2 px-10 mx-auto ">
            <InputLabel for="NameCust" value="گەڕان بۆ کڕیاری تاک" />
            <searchable-select
            :items="customersTak"
            :label="'گەڕان بۆ کڕیاری تاک'"
            v-model="searchCustId"
            :searchFields="['name_customer','phone_number','address']"
            class="w-full"
            :deselectable="true"
        ...
        <div class="mx-10 my-3 ">
            ...
            <TextInput
              id="NameCust"
              type="text"
              placeholder="ناوی کڕیاری کۆ بنووسە"
          ...
          <div class="mx-10 my-3 ">
            ...
            <TextInput
              id="NameCust"
              type="text"
              placeholder="ناوی کڕیاری تاک بنووسە"
              v-model="SearchCustomerTak"
              class="w-full mt-1"
            />
          </div>
        <div class="mx-10 my-3 ">
           ...
           <Table>
             <TableHead>
               <TableRowHead>
                 ...
                 <TableDataHead class="w-64">بابەت</TableDataHead>
             ...
             <TableBody class="bg-gray-100">
               ...
               class="cursor-pointer " @click="GotoInvoice(invoice)" v-for="invoice in invoices.data" :key="invoice.id">
                <TableData class="w-16 ">{{ invoice.getDebt ? invoice.getDebt.s_id : "" }}</TableData>
                 ...
                 <TableData class="w-64 ">{{ invoice.customer ? invoice.customer.name_customer : '' }}</TableData>
                 <TableData class="w-64">{{ invoice.name }}</TableData>
...
const SearchCustomer = ref(props.invoices.SearchCustomer);
const searchCustId = ref(props.invoices.searchCustId);
const searchProdId = ref(props.invoices.searchProdId);

const searchCateId = ref(props.invoices.searchCateId);
const SearchCustomerTak = ref(props.invoices.SearchCustomerTak);
...
watch([search , end_date, start_date,SearchProduct,SearchCustomer,searchCustId,searchProdId,searchCateId,SearchCustomerTak ], debounce(([newSearch,  newend_date , newstart_date,newSearchProduct,newSearchCustomer,newsearchCustId,newsearchProdId,newsearchCateId,newSearchCustomerTak ]) => {
  router.get(
    route("list.invoice.all"),
    {
    SearchProduct: newSearchProduct,
    SearchCustomer: newSearchCustomer,
    searchCustId: newsearchCustId,
    searchProdId: newsearchProdId,
    searchCateId: newsearchCateId,
    SearchCustomerTak: newSearchCustomerTak,
    search: newSearch,
