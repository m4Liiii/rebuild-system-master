...
<div class="justify-between md:flex ">
    <div class="flex items-center p-4 py-7">
        <Link
        :href="route('debtors.onlydebts.index', debtor.id)"
        ...
        <div class="flex flex-wrap justify-center flex-initial m-2 ">
          ...
          <div class="px-2 my-3 lg:w-1/4">
            ...
            <TextInput
              id="old_debt"
              type="text"
              :value="formatNumber(props.onlyDebt.old_debt)"
              class="w-full mt-1"
...
const debtor = usePage().props.debtor;
...
const createdDate = computed(() => {
  return format(new Date(props.onlyDebt.created_at), 'yyyy-MM-dd\'T\'HH:mm');
});
const form = useForm({
  old_debt: props.onlyDebt.old_debt,
  amount_of_debt: formatPrice(props.onlyDebt.amount_of_debt),
  discount: formatPrice(props.onlyDebt.discount),
  name_of_recipient: props.onlyDebt.name_of_recipient,
  notice_of_debt: props.onlyDebt.notice_of_debt,
  created_at: createdDate.value,
  type_amount: 0,
});

const submit = () => {
  form.put(route("debtors.onlydebts.update" , {debtor: debtor.id , onlydebt:props.onlyDebt.id} ,{
        preserveScroll: true,
        onError: (errors) => {
            form.errors = errors;
        },
