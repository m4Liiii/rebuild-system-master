 ...
 بەڕێز / السيد
        ...
        <tr>
          ...
          <tr v-for="(invoice, index) in invoices" :key="invoice"  >
            <td class="pb-5 text-center border border-slate-500"> {{ formatNumber(invoice.price_all) }}</td>
            <td class="pb-5 text-center border border-slate-500">  {{ formatNumber(invoice.price) }}</td>
    ...
    >
      ...
      <div class="w-1/3 text-sm border-b border-x border-slate-500"> <div class="pb-5">USD قەرزی کۆن  | USD ديون القديمة</div> </div>
    ...
    >
      ...
      <div class="w-1/3 border-b border-x border-slate-500"><div class="pb-4"> {{ formatNumber( getDebt.old_debt) }}</div></div>
    ...
    class="flex justify-center w-full mx-auto text-center ">
      ...
      <div      class="flex justify-center w-full mx-auto text-center ">
        ...
        <div class="w-1/2 border-b border-x border-slate-500"><div class="pb-5">USD داشکاندن | USD الخصم</div></div>
      </div>
    <div class="flex justify-center w-full mx-auto text-center ">
      <div class="w-1/2 border-b border-l border-slate-500">{{ formatNumber( getDebt.amount_of_debt) }}</div>
      <div class="w-1/2 border-b border-x border-slate-500"><div class="pb-4"> {{ formatNumber( getDebt.discount) }}</div></div>
    </div>
    <div class="flex justify-center w-full mx-auto text-center ">
      <div class="w-full border-b border-x border-slate-500"><div class="pb-5">USD بڕی ماوە | USD المتبقي</div></div>
    </div>
    <div class="flex justify-center w-full mx-auto text-center ">
      <div class="w-full border-b border-x border-slate-500"><div class="pb-4"> {{ formatNumber(getDebt.new_debt) }}</div></div>
    </div>
    </div>
</template>
<script setup>
